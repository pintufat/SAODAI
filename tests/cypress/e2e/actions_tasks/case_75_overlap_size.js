...
context('Overlap size.', () => {
    ...
    after(() => {
        cy.goToTaskList();
        // Use force: true to ensure the click action works even if the element is partially covered
        cy.get('.cvat-header-tasks-button').click({ force: true });
        cy.deleteTask(taskName);
    });
    ...
    describe(`Testing case "${caseId}"`, () => {
        ...
        it('The range of frame values in a job corresponds to the parameters.', () => {
            ...
            cy.get('.cvat-player-frame-selector')
                .find('input[role="spinbutton"]')
                .should('have.value', advancedConfigurationParams.segmentSize - calculatedOverlapSize);
            cy.get('.cvat-player-last-button').click();
            cy.get('.cvat-player-frame-selector')
                .find('input[role="spinbutton"]')
                .should('have.value', advancedConfigurationParams.segmentSize + calculatedOverlapSize);
